...
//enter with successesAvailable<popSize/2 and sampled<popSize/2
    ...
    oddsOfCardNew(successesObserved, sampled, successesAvailable, popSize) {
        ...
        // What are the sizes of the two sets we are intersecting, identified by
        // relative size?
    ...
    compute(deckSize, successesAvailable, cardsDrawn, successesWanted) {
        /*
            N choose K = ((N!)/(K!(N-K!))
            Given
        ...
        */
        ...
        if (cardsDrawn <= 0 || successesAvailable <= 0 || deckSize <= 0) {
            console.log("Parameters must be positive integers");
            Prob = 0;
        } else if (successesAvailable > deckSize || cardsDrawn > deckSize) {
            // Successes available or samples taken larger than actual population.
            console.log("successesAvailable and cardsDrawn must be less than deckSize");
            Prob = 0;
        } else if (successesWanted < 0 || successesWanted < cardsDrawn + successesAvailable - deckSize) {
            // We observed negative successes, or we pulled out so much of the
            // population that we would absolutely have had to have cardsDrawn more
            // succeses than this.
            Prob = 0;
        } else if (successesWanted >= cardsDrawn || successesWanted >= successesAvailable) {
            ...
            // math. But first, we have to precondition the input parameters

            if (2 * successesAvailable > deckSize) {
                // More than half of the population is successes.
                if (2 * cardsDrawn > deckSize) {
                    /*More than half the population has been cardsDrawn.
                    Change to a different problem:
                     ...
                     The deckSize - successesAvailable - cardsDrawn +
                     successesWanted has the +successesWanted to avoid double-
                     counting the successesAvailable that were also cardsDrawn in the
                     subtraction. So it gets the number of things that were
                     originally neither cardsDrawn nor observed.
                     P(the unsampled part contains some number of failures or
                     fewer) = P(the cardsDrawn part contains x successes or fewer)
                    */
                    Prob = this.oddsOfCardNew(deckSize - successesAvailable - cardsDrawn + successesWanted, deckSize - cardsDrawn, deckSize - successesAvailable, deckSize);
                } else {
                    /* Half or less of the population has been cardsDrawn.
                     ...
                     minus that.
                    */
                    Prob = 1 - this.oddsOfCardNew(cardsDrawn - successesWanted - 1, cardsDrawn, deckSize - successesAvailable, deckSize);
                }
            } else if (2 * cardsDrawn > deckSize) {
                /* Half or less of the population is successes, but we cardsDrawn more
                 than half of it.
                 ...
                 other tail.
                */
                Prob = 1 - this.oddsOfCardNew(successesAvailable - successesWanted - 1, successesAvailable, deckSize - cardsDrawn, deckSize);
            } else {
                // We're sampling half or less of the population, and half or less
                // of it is successes. Just go straight to the actual function.
                Prob = this.oddsOfCardNew(successesWanted, cardsDrawn, successesAvailable, deckSize)
            }
        }
